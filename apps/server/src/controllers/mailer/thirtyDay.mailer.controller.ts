...
export const ThirtyDayMail = async (
  ...
  try {
    ...
    reminderTargetStart.setDate(reminderTargetStart.getDate() + 30);
    ...
    if (!dueSubscriptions || dueSubscriptions.length === 0) {
      return res.status(200).json({
        success: true,
        message: "No subscriptions due for 30 day reminder today",
      });
    }
    ...
    return res.status(200).json({
      success: true,
      message: `30 day reminder email(s) sent for ${dueSubscriptions.length} subscription(s)`,
    });
  } catch (error) {
    console.log(`Error while sending 30 day mail: ${error}`);

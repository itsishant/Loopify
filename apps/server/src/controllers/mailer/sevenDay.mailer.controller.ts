...
export const SevenDayMail = async (
  ...
  try {
    ...
    reminderTargetStart.setDate(reminderTargetStart.getDate() + 7);
    ...
    if (!dueSubscriptions || dueSubscriptions.length === 0) {
      return res.status(200).json({
        success: true,
        message: "No subscriptions due for 7 day reminder today",
      });
    }
    ...
    return res.status(200).json({
      success: true,
      message: `7 day reminder email(s) sent for ${dueSubscriptions.length} subscription(s)`,
    });
  } catch (error) {
    console.log(`Error while sending 7 day mail: ${error}`);
